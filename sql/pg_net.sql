...
-- API: Private
create unlogged table http_request_queue(
    id bigserial,
...
create or replace function check_worker_is_up() returns void as $$
begin
...
$$ language plpgsql;
comment on function check_worker_is_up() is 'raises an exception if the pg_net background worker is not up, otherwise it doesn''t return anything';

-- Associates a response with a request
-- API: Private
create unlogged table _http_response(
    id bigint,
    status_code integer,
    content_type text,
    headers jsonb,
...
create index on _http_response (created);

-- Blocks until an http_request is complete
-- API: Private
create or replace function _await_response(
    request_id bigint
)
    returns bool
    language plpgsql
...
-- url encode a string
-- API: Private
create or replace function _urlencode_string(string varchar)
    -- url encoded string
    returns text
    language 'c'
    immutable
...
-- API: Private
create or replace function _encode_url_with_params_array(url text, params_array text[])
    -- url encoded string
    returns text
    language 'c'
    immutable
...
create or replace function worker_restart()
  returns bool
  language 'c'
as 'MODULE_PATHNAME';

create or replace function wait_until_running()
  returns void
  language 'c'
as 'MODULE_PATHNAME';
comment on function wait_until_running() is 'waits until the worker is running';

create or replace function wake()
  returns void
  language 'c'
as 'MODULE_PATHNAME';
...
declare
    ...
    params_array text[];
begin
    select coalesce(array_agg(_urlencode_string(key) || '=' || _urlencode_string(value)), '{}')
    into params_array
    ...
    values (
        'GET',
        _encode_url_with_params_array(url, params_array),
        headers,
...
begin
    ...
    select
        coalesce(array_agg(_urlencode_string(key) || '=' || _urlencode_string(value)), '{}')
    into
        params_array
    from
    ...
    -- Add to the request queue
    insert into http_request_queue(method, url, headers, body, timeout_milliseconds)
    values (
        'POST',
        _encode_url_with_params_array(url, params_array),
        headers,
...
-- API: Public
create type http_response_result as (
...
-- API: Private
create or replace function _http_collect_response(
    -- request_id reference
    request_id bigint,
...
begin
    ...
    if not async then
        perform _await_response(request_id);
    end if;
...
create or replace function http_collect_response(
    -- request_id reference
    request_id bigint,
    -- when `true`, return immediately. when `false` wait for the request to complete before returning
    async bool default true
...
as $$
begin
  raise notice 'The http_collect_response function is deprecated.';
