...
def test_wake_does_not_leak_xact_callbacks(sess, autocommit_sess):
    """
    wake() registers a transaction callback so the worker is woken at
    ...
    Otherwise every transaction that calls http_* leaks an entry in
    ...
    single-statement transactions that call wake(): it must stay flat.
    """
    ...
    for _ in range(50):
        autocommit_sess.execute(text("select wake()"))
    ...
    growth = after - before
    print(f"TopMemoryContext growth over {iterations} wake() transactions: {growth} bytes")
    ...
    assert growth < 32 * 1024, (
        ...
        f"transactions calling wake(), expected it to stay flat"
    )
