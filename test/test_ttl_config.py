...
def assert_pg_error(excinfo, expected_message, expected_detail):
    """
    Assert the exact primary message and DETAIL line of the PostgreSQL error
    wrapped in a SQLAlchemy DBAPIError, i.e. the full error a user would see:

        ERROR:  <expected_message>
        DETAIL:  <expected_detail>
    """
    diag = excinfo.value.orig.diag
    assert diag.message_primary == expected_message
    assert diag.message_detail == expected_detail
...
def test_invalid_ttl_is_rejected(autocommit_sess):
    ...
    for bad in ["1 blah", "yesterday", "abc", "1 minute 10 potatoes"]:
        ...
        # the complete error the user sees: the GUC error plus the interval
        # parser's own message as DETAIL
        assert_pg_error(
            excinfo,
            f'invalid value for parameter "pg_net.ttl": "{bad}"',
            f'invalid input syntax for type interval: "{bad}"',
        )
...
def test_compound_ttl_is_honored_by_worker(sess, autocommit_sess):
    ...
    once that time has passed and the worker wakes
    """
    try:
