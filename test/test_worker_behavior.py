...
def test_worker_writes_increment_pgstat_counters(sess, autocommit_sess):
    ...
    wait_until(
        fetch=lambda: sess.execute(
            text("select count(*) from net.http_request_queue;")
        ).fetchone(),
        predicate=lambda result: result[0] == 0,
        timeout=10,
        sleep_interval=0.5,
    ...
    (resp_count,) = autocommit_sess.execute(
        text("select count(*) from net._http_response;")
    ).fetchone()
...
def test_worker_idles_when_net_schema_exists_without_extension(sess, autocommit_sess):
    ...
    if changed:
        assert last_row is not None, (
            "pg_net worker is down, it crashed after seeing the net schema"
        )
        assert last_row[0] == pid, (
            "pg_net worker restarted, it's crash looping on the net schema"
        )
