...
5. **pg_net.max_timeout_ms** _(default: 600000)_: An integer that bounds the `timeout_milliseconds` of a request. The worker clamps any value outside `1..pg_net.max_timeout_ms` to it, so every request has a finite lifetime and a single request can't block the others. Only superusers can change it.
